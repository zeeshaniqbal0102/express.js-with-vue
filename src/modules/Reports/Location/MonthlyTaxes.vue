<template>
  <v-layout fill-height column="">
    <h3 v-if="isLoading" class="text-xs-center ma-2">Monthly Taxes</h3>
            ...
            columns: [
              {
                ...
                text: [
                  ...
                  {
                    text: `Monthly Taxes Page ${currentPage.toString()} of ${pageCount}`,
                  },
                ],
                alignment: 'center',
              },
  ...
  methods: {
    ...
    async loadReport() {
      ...
      try {
        ...
        // Sale  by Month
        if (this.report) {
          const paymentMethodsHeaders = [[{ text: 'Months', colSpan: 2 }, '', 'Total']]

          const paymentMethodsBody = await Promise.all(
            this.report.map(month => [
              {
                text: `${month.name} ${this.$moment(month.start).format(
                  'MM/DD/YY',
                )} - ${this.$moment(month.end).format('MM/DD/YY')}`,
                colSpan: 2,
              },
              '',
          ...
          await Promise.all(
            this.report.map(async month => {
              ...
              if (month.data.length > 0) {
                this.doc.content.push({
                  stack: [
                    `${month.name} ${this.$moment(month.start).format(
                      'MM/DD/YY',
                    )} - ${this.$moment(month.end).format('MM/DD/YY')} ${
                      month.totals && month.totals.revenue
                        ? `Revenue: ${month.totals.revenue}`
                        : ''
                    } ${
                      month.totals && month.totals.taxCollected
                        ? `Tax Collected: ${month.totals.taxCollected}`
                        : ''
                    }`,
                    {
                      table: {
      ...
      if (this.$v.$invalid) {
        ...
        pdfMake.createPdf(this.doc).getDataUrl(data => {
          ...
          const newFile = this.dataURLtoFile(
            data,
            `Payments_${this.location.code}.pdf`,
